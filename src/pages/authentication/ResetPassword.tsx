...
const ResetPasswordPage: React.FC = () => {
  ...
  const handleSubmit = async (values: { new_password: string; confirmPassword: string }) => {
    setLoading(true); // Set loading to true
    try {
      const response = await axios.post(`${apiURL}/reset_password.php?token=${token}`, values);
        ...
        >
          {() => (
            <Form className="space-y-4">
              {/* New Password */}
              <div>
                <label htmlFor="new_password" className="block text-sm font-medium text-pry">
                  New Password
                </label>
                <Field
                  id="new_password"
